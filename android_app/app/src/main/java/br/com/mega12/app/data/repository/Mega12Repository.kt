...
class Mega12Repository(private val preferencesManager: PreferencesManager) {
    ...
    suspend fun updateFiscalConfig(config: FiscalConfig): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            val response = api.updateFiscalConfig(config)
    ...
    suspend fun saveAvaria(avaria: AvariaItem): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            val response = api.saveAvaria(avaria)
            Result.success(response.isSuccessful)
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    suspend fun updateInstallment(id: String, updates: Map<String, Any?>): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            val response = api.updateInstallment(id, updates)
            Result.success(response.isSuccessful)
        } catch (e: Exception) {
            Result.failure(e)
        }

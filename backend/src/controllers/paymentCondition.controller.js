...
class PaymentConditionController {
  async list(req, res, next) {
    try {
      ...
      const list = await paymentConditionRepository.findAll({ onlyActive });
  ...
  async getById(req, res, next) {
    try {
      const item = await paymentConditionRepository.findById(req.params.id);
      if (!item) {
  ...
  async save(req, res, next) {
    try {
      ...
      const saved = await paymentConditionRepository.upsert(req.body);
  ...
  async delete(req, res, next) {
    try {
      await paymentConditionRepository.delete(req.params.id);

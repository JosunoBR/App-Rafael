...
class FiscalPresetRepository {
  async findAll() {
    const rows = await queryAll("SELECT * FROM fiscal_presets ORDER BY isDefault DESC, name ASC");
    return rows.map(r => this._hydrate(r));
  }

  async findById(id) {
    const row = await queryOne("SELECT * FROM fiscal_presets WHERE id = ?", [id]);
  ...
  async delete(id) {
    ...
    await execute("DELETE FROM fiscal_presets WHERE id = ?", [id]);
    return true;
  }

...
class BackupRestoreService {
  async restoreFromBackupData(rawBackup) {
    ...
    for (const sup of supplierMap.values()) {
      const check = db.exec(`SELECT id FROM suppliers WHERE razaoSocial = ?`, [sup.razaoSocial]);
    ...
    if (Array.isArray(productsList)) {
      for (const prod of productsList) {
        ...
        if (!check[0] || check[0].values.length === 0) {
          ...
          `, [
            prod.id || `prd_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
            prod.codigo,
            prod.descricao,
    ...
    for (const ord of ordersToRestore) {
      ...
      const check = db.exec(`SELECT id FROM purchase_orders WHERE numeroPedido = ?`, [num]);
      if (!check[0] || check[0].values.length === 0) {
        const orderId = ord.header.id || `ord_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
        ...
        if (Array.isArray(ord.items)) {
          for (const it of ord.items) {
            ...
            `, [
              it.id || `it_${orderId}_${Math.random().toString(36).slice(2, 6)}`,
              orderId,
              it.codigo || '',

...
class ExportService {
  ...
  // Formato: A4 Retrato | SEM NENHUMA LOJA
  // ==========================================
  generateCommercialOrderPdf(order) {
    ...
    let finalY = (doc.lastAutoTable?.finalY || 180) + 5;
    ...
    const descValor = (order.items || []).reduce((acc, i) => acc + (Number(i.valorDescontoTotal) || 0), 0);
    doc.text(`• Desconto Comercial: ${descOff > 0 ? `${descOff}% (- R$ ${descValor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })})` : 'R$ 0,00'}`, 75, finalY + 16);
    const stValor = (order.items || []).reduce((acc, i) => acc + (Number(i.valorStTotal) || 0), 0);
    doc.text(`• Impostos / ST: ${stValor > 0 ? `+ R$ ${stValor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'Incluso / Isento'}`, 75, finalY + 21);
    ...
    if (order.header?.observacoes) {
      doc.text(`5. Obs: ${order.header.observacoes}`, 16, noticeY + 26.5);
    }

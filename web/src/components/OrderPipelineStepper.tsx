...
export const OrderPipelineStepper: React.FC<OrderPipelineStepperProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-4 mb-5 transition-all">
      {/* Cabeçalho de Identificação do Pedido na Esteira */}
      <div className="flex items-center gap-2 mb-3 pb-2.5 border-b border-slate-100 dark:border-slate-800/80 text-xs">
        <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500">Esteira Operacional:</span>
        <span className="px-2.5 py-0.5 rounded-lg bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 font-mono font-black border border-emerald-500/30">
          {order.header.numeroPedido || 'PED-NOVO'}
        </span>
        {order.header.fornecedor && (
          <>
            <span className="text-slate-300 dark:text-slate-600">•</span>
            <span className="font-bold text-slate-700 dark:text-slate-300 truncate max-w-sm">{order.header.fornecedor}</span>
          </>
        )}
      </div>
